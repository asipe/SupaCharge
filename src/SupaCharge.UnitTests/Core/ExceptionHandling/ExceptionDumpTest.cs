...
namespace SupaCharge.UnitTests.Core.ExceptionHandling {
  ...
  public class ExceptionDumpTest : BaseTestCase {
    ...
    public void TestLengthyChainOfExceptions() {
      ...
      Assert.That(split[3], Is.EqualTo("----- Inner Exception"));
      Assert.That(split[4], Is.EqualTo("System.Exception"));
      Assert.That(split[5], Is.EqualTo("bye"));
      Assert.That(split[7], Is.EqualTo("----- Inner Exception"));
      Assert.That(split[8], Is.EqualTo("System.Exception"));
      Assert.That(split[9], Is.EqualTo("Here"));
      Assert.That(split[11], Is.EqualTo("----- Inner Exception"));
      Assert.That(split[12], Is.EqualTo("System.Exception"));
      Assert.That(split[13], Is.EqualTo("yo"));
      Assert.That(split[15], Is.EqualTo("----- Inner Exception"));
      Assert.That(split[16], Is.EqualTo("System.Exception"));
      Assert.That(split[17], Is.EqualTo("last one"));
    }
